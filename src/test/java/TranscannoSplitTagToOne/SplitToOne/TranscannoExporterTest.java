...
import TranscannoSplitTagToOne.SplitToOne.TranscannoExporter;
 ...
 * {@link TranscannoExporter} class. Feel free to adapt and enhance this test class
 ...
 */
public class TranscannoExporterTest extends PepperExporterTest {
	/**
	...
	@Before
	public void setUp() {
		setFixture(new TranscannoExporter());
	...
	public void test_DummyImplementation() {
		...
		getFixture().setCorpusDesc(new CorpusDesc().setCorpusPath(URI.createFileURI(getTempPath("TranscannoExporter").getAbsolutePath())));

		// starts the Pepper framework and the conversion process
		start();

		File superCorpus = new File(getTempPath("TranscannoExporter").getAbsolutePath() + "/rootCorpus");
